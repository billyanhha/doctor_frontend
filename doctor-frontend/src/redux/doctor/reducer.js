...
export const doctorReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case GET_APPOINTMENTS_FROM_TO_SUCCESSFUL:
            state = {
                ...state,
                appointmentTimeTable: action.appointmentFromTo,
            };
            return state;
        ...
        case GET_DOCTOR_DETAIL_SUCCESSFUL:
            ...
            return state;
        case GET_DOCTOR_ALL_RATING_SUCCESSFUL:
            state = { ...state, allRating: action?.data };
            return state;
        ...
        case DOCTOR_GET_ALL_SERVICE_REQUEST_SUCCESSFULLY:
            ...
            for (let i = 0; i < action?.serviceRequest?.length; i++) {
                ...
                let d = i;
                data[i].key = d;
                data[i].createdAt = handleCreated_at(
                    action?.serviceRequest?.[i]?.createdAt
                );
            }
